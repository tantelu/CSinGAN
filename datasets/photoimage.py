...
class PhotoData(data.Dataset):
    ...
    def __init__(self, data_dir, is_train, transform=None, singlemode=True, img_to_use=-999):
        ...
        if singlemode:
            if img_to_use == -999:
                self.num_data = len(glob(os.path.join(self.image_dir, '*.png')))
                print(os.path.abspath(self.image_dir))
                randidx = np.random.randint(0, self.num_data)
                self.randidx = randidx
                self.image_dir = sorted(glob(os.path.join(self.image_dir, '*.png')))[randidx]
            else:
                self.image_dir = sorted(glob(os.path.join(self.image_dir, '*.png')))[img_to_use]
        else:
            self.image_dir = sorted(glob(os.path.join(self.image_dir, '*.png')))
